...
for i in n_estimators:
    ...
    for j in max_depth:
        ...
        for k in min_samples_leaf:
            ...
            start = time.time()
            rf = RandomForestRegressor(max_depth=max_depth_tmp, n_estimators=n_estimators_tmp, min_samples_leaf=min_samples_leaf_tmp, n_jobs=11)
            # est, est_errors, train_score[jj], test_score[jj] = ml_train.train_est(rf, 'rf_str',f_scl[0:ind_data, :],o_scl[0:ind_data, :],tf_scl[0:ind_test, :],to_scl[0:ind_test, :],do_nn)
            est, est_errors, train_score[jj], test_score[jj] = ml_train.train_est(rf, 'rf_str',f_scl[0:ind_data, Output_ind1:Output_ind2],o_scl[0:ind_data, Output_ind1:Output_ind2],tf_scl[0:ind_test, Output_ind1:Output_ind2],to_scl[0:ind_test, Output_ind1:Output_ind2],do_nn)

            jj = jj + 1
            # print("train score:", train_score[j], "test score: ", test_score[j])
...
# plt.xlabel("Training examples")
# plt.ylabel("Score")
# data_part = [0.001,0.003, 0.01,0.03,0.1,0.3,0.5713,0.7,0.99]
...
#     est, est_errors, train_score[batch_i], test_score[batch_i] = ml_train.train_est(rf, 'rf_str', f_scl[0:ind_data,:], o_scl[0:ind_data,:], tf_scl[0:ind_test,:], to_scl[0:ind_test,:], do_nn)
#
...
pickle.dump([n_estimators,train_score, test_score], open(figpath + 'leaf_sensitivity_Rsq_RFdiff' + str(Output_ind1) + '_' + str(Output_ind2) + '.pkl', 'wb'))
