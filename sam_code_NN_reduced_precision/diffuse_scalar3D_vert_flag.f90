subroutine diffuse_scalar3D_vert_flag (field,fluxb,fluxt,tkh_xy,tkh_z,rho,rhow,flux)
...
do j=1,ny
 do i=1,nx	
!  if(dorandomforest.and.do_rf_diffusion) then
!   flx(i,j,0) = 0.
!  else
...
do k=1,nzm
 ...
 do j=1,ny
  do i=1,nx		
   !if(dorandomforest.and.do_rf_diffusion) then !Yani
   ! flx(i,j,k) = 0.0 !This predicted by RF - Yani
  ! elseif(dorandomforest.and.do_rf_diffusion) then 
  !  if(kb==0) then
  !   flx(i,j,0) = flx(i,j,0) !Keep only surface fluxes - Yani - need to think of half levels ? 
  !  else
  !   flx(i,j,k) = 0.0
   !else 
   ! flx(i,j,k) = flx(i,j,k) ! keep fluxes if RF didnt predict the vertical diffusion 
   !end if
   if (dorandomforest.and.do_rf_diffusion) then !#Yani
    dfdt(i,j,k)=dtn*(dfdt(i,j,k)) !-(flx(i,j,k)-flx(i,j,kb))*rhoi)
  !   if (i==1.and.j==1.and.k==1) then
  !      print *, "went to the right place"
  !   end if
   else
    dfdt(i,j,k)=dtn*(dfdt(i,j,k)-(flx(i,j,k)-flx(i,j,kb))*rhoi)       
   end if
...
end subroutine diffuse_scalar3D_vert_flag
