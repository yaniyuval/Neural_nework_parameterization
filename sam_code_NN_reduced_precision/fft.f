...
c
      ...
      i = ifax(1)
cgsp  if (ifax(i+1) .gt. 5 .or. n .le. 4) ifax(1) = -99
cgsp  if (ifax(1) .le. 0 )call uliber(33, ' fftfax -- invalid n', 20)
...
cdir$ ivdep
  ...
  160 continue
	...
	subroutine cosft(a,work,trigs,ifax,inc,jump,n,lot,isign)
	implicit none
	real a(*),work(*),trigs(*)
	...
	if(isign.eq.-1)then 
	   ...
	   call fft991(a,work,trigs,ifax,inc,jump,n,lot,isign)
           ...
           do ilot = 1,lot
             ...
             do i=3,n
               y(i) = a(jump1+(i-1)*inc)*float(n)
             end do ! 
	     y(1) = a(jump1)*float(n)
	     y(2) = a(jump1+(n+1-1)*inc)*float(n)
