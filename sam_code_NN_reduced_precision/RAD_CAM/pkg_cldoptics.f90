...
! jrm Reworked effective radius algorithm
          ...
          rel(i,k) = rliqland + (rliqocean-rliqland) * min(1.0_r4,max(0.0_r4,(tmelt-t(i,k))*0.05))
          ! Modify for snow depth over land
          rel(i,k) = rel(i,k) + (rliqocean-rel(i,k)) * min(1.0_r4,max(0.0_r4,snowh(i)*10.))
          ! Ramp between polluted value over land to clean value over ocean.
          rel(i,k) = rel(i,k) + (rliqocean-rel(i,k)) * min(1.0_r4,max(0.0_r4,1.0-landm(i)))
