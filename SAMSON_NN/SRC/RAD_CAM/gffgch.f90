...
! Input arguments
!
   real(r4), intent(in) :: t          ! Temperature
!
! Output arguments
!
   integer, intent(inout) :: itype   ! Flag for ice phase and associated transition

   real(r4), intent(out) :: es         ! Saturation vapor pressure
!
!---------------------------Local variables-----------------------------
!
   real(r4) e1         ! Intermediate scratch variable for es over water
   real(r4) e2         ! Intermediate scratch variable for es over water
   real(r4) eswtr      ! Saturation vapor pressure over water
   real(r4) f          ! Intermediate scratch variable for es over water
   real(r4) f1         ! Intermediate scratch variable for es over water
   real(r4) f2         ! Intermediate scratch variable for es over water
   real(r4) f3         ! Intermediate scratch variable for es over water
   real(r4) f4         ! Intermediate scratch variable for es over water
   real(r4) f5         ! Intermediate scratch variable for es over water
   real(r4) ps         ! Reference pressure (mb)
   real(r4) t0         ! Reference temperature (freezing point of water)
   real(r4) term1      ! Intermediate scratch variable for es over ice
   real(r4) term2      ! Intermediate scratch variable for es over ice
   real(r4) term3      ! Intermediate scratch variable for es over ice
   real(r4) tr         ! Transition range for es over water to es over ice
   real(r4) ts         ! Reference temperature (boiling point of water)
   real(r4) weight     ! Intermediate scratch variable for es transition
   integer itypo   ! Intermediate scratch variable for holding itype

   real(r4), parameter :: tmelt = 273.16
!
!-----------------------------------------------------------------------
!
