...
C-----------------------------------------------------------------------
c
c $Id: radtpl.f,v 1.1.1.1 2007/01/11 22:58:18 cw Exp $
c
...
C
      ...
      real ddy                   ! Thickness of layer for tmp interp
...
C at the full level below less ddy*delta t,between the full level
C
      do k=2,plev
        do i=1,plon
          ddy = (piln(i,k) - pmln(i,k))/(pmln(i,k-1) - pmln(i,k))
          tint(i,k)  = tnm(i,k) - ddy*(tnm(i,k)-tnm(i,k-1))
