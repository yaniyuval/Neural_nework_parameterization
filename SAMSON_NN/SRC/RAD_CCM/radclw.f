      subroutine radclw(doabsems,lwupcgs ,tnm     ,qnm     ,o3vmr   ,
     $                  absnxt  ,abstot  ,emstot  ,	
     $                  pmid    ,pint    ,pmln    ,piln    ,plco2   ,
     $                  plh2o   ,n2o     ,ch4     ,cfc11   ,cfc12   ,
     $                  cld     ,tclrsf  ,qrl     ,ful     ,fdl     ,
...
C
c
c Adopted for LES by Marat Khairoutdinov, December 1998
C-----------------------------------------------------------------------
...
C                               
      ...
      real absnxt(plond,plevp,4) ! Nearest layer absorptivities
...
C
      do k=1,plev
        do i=1,plon
          fclb4(i,k) = 0.
          fclt4(i,k) = 0.
           tclrsf(i,k+1) = max(tclrsf(i,k)*(1. - cld(i,k+1)),1.e-35)
           rtclrsf(i,k+1) = 1./tclrsf(i,k+1)
!          tclrsf(i,k+1) = tclrsf(i,k)*(1. - cld(i,k+1))
!          rtclrsf(i,k+1) = 1./(1.e-20+tclrsf(i,k+1))
        end do
      end do
...
CDIR$ IVDEP
        do ii=1,npts
          ...
          if (km.le.khiv(i)) then
            tmp1 = (tclrsf(i,plev)*rtclrsf(i,km2))*cld(i,km2)
...
C
        flwds(i) = fdl(i,plevp)*1.e-3
C
C Net flux
C
        flns(i) = (ful(i,plevp) - fdl(i,plevp))*1.e-3
C
C Clear sky flux at top of atmosphere
C
C# Changed definition of top of atmosphere net clear sky outgoing lw flux
C# to match the top of atmosphere net outgoing lw flux in clear areas.
C# This will give zero lw cloud forcing in clear sky regions.
C# Peter Blossey, March 2004.
c$$$        flntc(i) = fsul(i,1)*1.e-3
        flntc(i) = (fsul(i,1)     - fsdl(i,1)    )*1.e-3
        flnsc(i) = (fsul(i,plevp) - fsdl(i,plevp))*1.e-3
C
C Outgoing ir
C
        flnt(i) = (ful(i,1) - fdl(i,1))*1.e-3
      end do
C
