...
C-----------------------------------------------------------------------
c
c $Id: radcsw.f,v 1.1.1.1 2007/01/11 22:58:18 cw Exp $
c
...
C
      ...
      real fsu(plond,plevp)    ! Total upwelling flux	
      real fsd(plond,plevp)    ! Total downwelling flux	
      real fsns(plond)         ! Surface absorbed solar flux
      real fsnt(plond)         ! Flux Shortwave Downwelling Top-of-atmos
      real fsntm(plond)        ! Flux Shortwave Downwelling Top-of-model
...
C
      ...
      ie(1) = isrchfle(plon-is(1),
     &		coszrs(min(plon,is(1)+1)),1,0.0) + is(1) - 1
      nloop = 1
C
C Possibly 2 daytime loops needed:
C
      if (ie(1).ne.plon) then
        is(2) = isrchfgt(plon-ie(1),
     &		coszrs(min(plon,ie(1)+1)),1,0.0) + ie(1)
...
C     # Compute toa net downward sw flux at top-of-atmosphere.
            fsntm(i) = fsntm(i) + solflx(i)*(fluxdn(i,1) - fluxup(i,1))
            fsnt(i) = fsnt(i) + solflx(i)*(fluxdn(i,0) - fluxup(i,0))
