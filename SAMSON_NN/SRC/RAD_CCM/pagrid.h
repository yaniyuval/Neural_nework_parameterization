c
c $Id: pagrid.h,v 1.1.1.1 2007/01/11 22:58:18 cw Exp $
c $Author: cw $
c
C
...
C
      parameter(pbflnb=(9 + 2*pcnst + 1*pnats)*plndlv + (7+pcnst)*plond)
C
C In pbflna, there are 6 multi-level and 3 single-level fields.
C
      parameter(pbflna = (3 + 6*plev)*plond)
      parameter(pbflnm1 = (3 + 3*plev)*plond)
