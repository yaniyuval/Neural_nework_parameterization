...
real f(dimx1:dimx2, dimy1:dimy2, dimz),fm(nzm), factor
integer i,j,k	
factor = 1./(nx*ny)
do k =1,nzm
 fm(k) = 0.
...
implicit none
integer dimx1, dimx2, dimy1, dimy2, dimz
real f(dimx1:dimx2, dimy1:dimy2, dimz),fm(nzm),fm1(nzm),factor
integer i,j,k	
factor = 1./(nx*ny)
do k =1,nzm
 fm(k) = 0.
...
if(dompi) then
 ...
 call task_sum_real(fm1,fm,nzm)
 do k=1,nzm
  fm(k)=fm(k)/float(nsubdomains)
 end do
endif
end
