subroutine diffuse_scalar_vert_flag (f,fluxb,fluxt, &
                          fdiff,flux,f2lediff,f2lediss,fwlediff,doit)

use grid
use vars, only: tkh_xy, tkh_z, rho, rhow
use mse       ! peters
use params    ! peters
implicit none
...
if(RUN3D) then
  call diffuse_scalar3D_vert_flag (f,fluxb,fluxt,tkh_xy,tkh_z,rho,rhow,flux)
else  
...
end subroutine diffuse_scalar_vert_flag
